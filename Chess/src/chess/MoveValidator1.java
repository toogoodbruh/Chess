...
public class MoveValidator1 {
	...
	public static boolean isCheckAndCheckmate(String sourceSquare, String destinationSquare, ArrayList<ReturnPiece> piecesOnBoard, Chess.Player currentPlayer) {
	    ...
	    for (ReturnPiece opponentPiece : piecesOnBoard) {
	        ...
	        if (opponentPiece.pieceType != king.pieceType
	            ...
	            if (!hasLegalMovesToEscapeCheck(sourceSquare, destinationSquare, piecesOnBoard, king, currentPlayer)) {
	...
	private static boolean hasLegalMovesToEscapeCheck(String sourceSquare, String destinationSquare, ArrayList<ReturnPiece> piecesOnBoard, ReturnPiece king,
	        ...
	        if (piece.pieceType != king.pieceType && piece.pieceType != getOpponentPawnType(currentPlayer)) {
	            ...
	            for (char file = 'a'; file <= 'h'; file++) {
	                for (int rank = 1; rank <= 8; rank++) {
	                    destinationSquare = "" + file + rank;

	                    // Check if the move is valid
	                    if (isValidMove(piece, piece.pieceFile.toString() + piece.pieceRank, destinationSquare,
	                            piecesOnBoard)) {
	                        // Simulate the move
	                        ReturnPiece movedPiece = piece;
	                        movedPiece.pieceFile = ReturnPiece.PieceFile.valueOf("" + file);
	                        movedPiece.pieceRank = rank;
	                        ArrayList<ReturnPiece> updatedBoard = processRegularMove(sourceSquare, piece.pieceFile.toString() + piece.pieceRank, piece, piecesOnBoard);
